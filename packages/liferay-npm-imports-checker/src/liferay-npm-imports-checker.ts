...
import {FilePath, Project, isNodeCoreModule} from 'liferay-js-toolkit-core';
...
export default function main(args): void {
	...
	const results: Results = checkProjects(projects);
	...
	globby
		...
		.forEach((pkgJsonPath) => {
			...
			try {
				...
				const npmbundlerrc: any = new Project(pkgJsonDir).npmbundlerrc;
 ...
 */
function checkProjects(projects): Results {
	...
	Object.values(projects).forEach((project: any) => {
		...
		const npmbundlerrc = project.npmbundlerrc;
