				...
				off: function(event, callback) {
					...
					if (listeners) {
						...
						if (callbackIndex > -1) {
							...
							console.warn(
								'Off: callback was not removed: ' +
									callback.toString()
							);
						}
					} else {
						console.warn(
							'Off: there are no listeners for event: ' + event
						);
					}
				},
 ...
 */
     ...
     */
				mapModule: function(module, contextMap) {
					...
					if (contextMap) {
						modules = modules.map(
							this._getModuleMapper(contextMap)
						);
					}
					...
					for (var alias in maps) {
						...
						if (Object.prototype.hasOwnProperty.call(maps, alias)) {
							...
							if (!aliasValue.exactMatch) {
								...
								if (
									module === alias ||
									module.indexOf(alias + '/') === 0
								) {
									return (
										aliasValue +
										module.substring(alias.length)
									);
								}
							}
						}
					...
					if (conditionalModules && !module.conditionalMark) {
						...
						for (var i = 0; i < conditionalModules.length; i++) {
							var conditionalModule =
								modules[conditionalModules[i]];

							if (
								this._queue.indexOf(conditionalModule.name) ===
									-1 &&
					...
					for (var i = 0; i < dependencyParts.length; i++) {
						...
						if (dependencyPart === '.') {
							...
							if (moduleParts.length) {
								...
								moduleParts = moduleParts.concat(
									dependencyParts.slice(i)
								);
 ...
 */
					...
					if (Array.isArray(arguments[0])) {
						...
						for (i = 0; i < arguments.length; ++i) {
							if (typeof arguments[i] === 'string') {
								...
								failureCallback = typeof arguments[++i] ===
									'function'
									? arguments[i]
					...
					return new Promise(function(resolve, reject) {
						...
						if (module.exports) {
							...
							if (!!exportedValue) {
								...
								var onScriptLoaded = function(loadedModules) {
									if (
										loadedModules.indexOf(moduleName) >= 0
									) {
										self.off(
											'scriptLoaded',
											onScriptLoaded
     ...
     */
				_getConfigParser: function() {
					...
					if (!this._configParser) {
						this._configParser = new global.ConfigParser(
							this._config
						);
					}
     ...
     */
				_getURLBuilder: function() {
					/* istanbul ignore else */
					if (!this._urlBuilder) {
						this._urlBuilder = new global.URLBuilder(
							this._getConfigParser()
						);
					}
				...
				_filterModulesByProperty: function(moduleNames, property) {
					...
					for (var i = 0; i < moduleNames.length; i++) {
						...
						var registeredModule =
							registeredModules[moduleNames[i]];
     ...
     */
				_loadModules: function(moduleNames) {
					...
					return new Promise(function(resolve, reject) {
						...
						var modulesForLoading = self._filterModulesByProperty(
							moduleNames,
							['requested', 'pendingImplementation']
						);
				...
				_loadScript: function(modulesURL) {
					...
					return new Promise(function(resolve, reject) {
						...
						script.onload = script.onreadystatechange = function() {
							...
							if (
								!this.readyState ||
								/* istanbul ignore next */ this.readyState ===
									'complete' ||
								/* istanbul ignore next */ this.readyState ===
									'load'
							) {
     ...
     */
				_reportMismatchedAnonymousModules: function(msg) {
					var errorMessage =
						'Mismatched anonymous define() module: ' + msg;
					var reportLevel = this._config
					...
					for (var i = 0; i < modules.length; i++) {
						...
						for (var j = 0; j < module.dependencies.length; j++) {
							...
							if (dependency === 'exports') {
								dependencyImplementations.push(
									moduleImpl.exports
								);
						...
						if (
							typeof module.pendingImplementation === 'function'
						) {
					...
					return new Promise(function(resolve, reject) {
						...
						for (var i = 0; i < moduleNames.length; i++) {
							modulesPromises.push(
								self._waitForModule(moduleNames[i])
							);
						}
...
(function() {
	if (
		...
		typeof window.ES6Promise !== 'undefined'
	) {
